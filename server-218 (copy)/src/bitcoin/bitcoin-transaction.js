//-------This is the complete code of bitcoin-transaction npm package,We are using code of npm package instead of package due to security reason
//---------------------------------------------------------------------------
...
var providers = {
	...
	balance: {
		mainnet: {
			...
			blockchain: function (addr) {
				...
				return request.get('https://blockchain.info/q/addressbalance/' + addr).send().then(function (res) {
					console.log("This is responce:"+res.body);
		...
		testnet: {
			blockexplorer: function (addr) {
				return request.get('https://www.blockchain.com/btctest/address/' + addr).send().then(function (res) {
					console.log(res);
	...
	utxo: {
		...
		testnet: {
			blockexplorer: function (addr) {
				return request.get('https://api.bitcore.io/api/BTC/testnet/address/' + addr + '?unspent=true&limit=1000').send().then(function (res) {
					return res.body.map(function (e) {
						return {
							txid: e.mintTxid,
							vout: e.mintIndex,
...
function getBalance (addr, options) {
	...
	return options.balanceProvider(addr).then(function (balSat) {
		console.log(balSat/BITCOIN_SAT_MULT);
...
function sendTransaction (options) {
	...
	return Promise.all([
      ...
      var utxo = utxos[i];
      console.log("utxo.confirmations:"+utxo.confirmations,"options.minConfirmations:"+options.minConfirmations);
   ...
   console.log("availableSat:"+availableSat,"amount of satoshi:"+amtSatoshi);
		...
		console.log("transaction id 1:"+msg);
		
		if (options.dryrun) {
			console.log("transaction id 2:"+msg);
			
			return msg;
		} else {
			console.log("transaction id 3:"+options.pushtxProvider(msg));
