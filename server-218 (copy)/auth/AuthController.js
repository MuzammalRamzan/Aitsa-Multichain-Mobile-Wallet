...
router.post('/verifyCode', async function (req, res, next) {  // Code Verification for Registration
  ...
  User.findOne({ walletId: req.body.username }, function (err, user) {
    ...
    if (req.body.code == user.verifyToken) {
      ...
      User.updateOne({ walletId: req.body.username }, { status: true }, async function (err, res) {
        if (err) {
      ...
      return res.status(200).send({ "status": true, "msg": "User has verified " });
    }
    else {
...
router.post('/changePassword', VerifyToken, function (req, res) {     // Update Password When User Enter Password
  ...
  try {
    ...
    User.findOne({ _id: userId }, function (err, user) {
      var defaultToken = jwt.sign({ id: user._id }, config.secret, {
        expiresIn: 86400 // expires in 24 hours
      });
      console.log("Find User " + user);
      ...
      console.log("Calling Update User ")
      User.updateOne({ _id: userId }, { password: hashedPassword }, function (err, res) {
        if (err) {
          console.log("Some Issues in mongdb");
          return res.status(400).send({ "success": false, "msg": "DB Issue" });
        }
